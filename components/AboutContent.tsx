  ...
  return (
    <div className="absolute left-[-80px]  2xl:left-[-120px] flex justify-between  items-center w-11/12 h-1/2 ">
      <div className="  h-full flex items-center   ">
        ...
        <span className="ml-20 h-1/2 w-[6px] rounded bg-black"></span>
      </div>
      <div className=" mx-10 w-[50%] flex flex-col">
        <h1 className="leading-none font-bold 2xl:text-[40px] text-[30px] uppercase">
          {props.title}
        </h1>
        <p className="2xl:text-[27px] text-[23px] mt-5">
          <Typewriter
            options={{ cursor: "|", delay: 30 }}
            onInit={(typewriter) => {
              typewriter.typeString(props.content).start();
            }}
          />
        </p>
        {props.joinButton && (
